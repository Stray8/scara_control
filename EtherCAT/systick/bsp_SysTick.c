...
#define SystemCoreClock_INC 50//定时器时钟周期为50us
...
void SysTick_Init(void)
{
	 ...
	 */
	if (SysTick_Config(SystemCoreClock / 20000))  //50us
	{ 
		/* Capture error */ 
		while (1);
	}
...
{ 
	TimingDelay = nTime * 1000 / SystemCoreClock_INC + 1;
...
//	}
	...
	sys_timer.usecL += SystemCoreClock_INC;
	if(sys_timer.usecL >= USECS_PER_HIGH)
	{
		++sys_timer.usecH;
		sys_timer.usecL -= USECS_PER_HIGH;
	...
	if(TimingDelay > SystemCoreClock_INC)
	{ 
		TimingDelay -= SystemCoreClock_INC;
	}
	else
	{
		TimingDelay = 0;
