...
void ecx_packeterror(ecx_contextt *context, uint16 Slave, uint16 Index, uint8 SubIdx, uint16 ErrorCode)
{
   ...
   *(context->ecaterror) = TRUE;
   ...
   Ec.data.str.ErrorCode = ErrorCode;
    ...
    uint8 b1, uint16 w1, uint16 w2)
{
   ...
   Ec.data.str.ErrorCode = ErrorCode;
   Ec.data.str.ErrorReg = (uint8)ErrorReg;
   Ec.data.str.b1 = b1;
   Ec.data.str.w1 = w1;
   Ec.data.str.w2 = w2;
   ecx_pusherror(context, &Ec);
...
int ecx_send_processdata_group(ecx_contextt *context, uint8 group)
{
   ...
   uint16 slave = 0;
   if(cycle_queue_element_count(&(context->port->pdo_send_queue)) >=2)return 0;
   
   /* find CoE and SoE mapping of slaves in multiple threads */
    for (slave = 1; slave <= *(context->slavecount); slave++){
		if (!group || (group == context->slavelist[slave].group)){
			if(context->slavelist[slave].proc_read_write) /* only if registered */{
				context->slavelist[slave].proc_read_write(context,slave);         
			}
        }
    }
