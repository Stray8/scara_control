...
char* ecx_elist2string(ecx_contextt *context)
{
   ...
   {
      //sprintf(timestr, "Time:%12.3f", Ec.Time.usecH + (Ec.Time.usecL / USECS_PER_HIGH) );
      switch (Ec.Etype)
      {
         case EC_ERR_TYPE_SDO_ERROR:
         {
//            sprintf(estring, "%s SDO slave:%d index:%4.4x.%2.2x error:%8.8x %s\n", 
//                    timestr, Ec.Slave, Ec.Index, Ec.SubIdx, Ec.data.AbortCode, ec_sdoerror2string(Ec.data.AbortCode));            
            break;
         }
         case EC_ERR_TYPE_EMERGENCY:
         {   
//            sprintf(estring, "%s EMERGENCY slave:%d error:%4.4x\n", 
//                    timestr, Ec.Slave, Ec.data.str.ErrorCode);            
            break;
         }
         case EC_ERR_TYPE_PACKET_ERROR:
         {
//            sprintf(estring, "%s PACKET slave:%d index:%4.4x.%2.2x error:%d\n", 
//                    timestr, Ec.Slave, Ec.Index, Ec.SubIdx, Ec.data.str.ErrorCode);            
            break;
         }
...
//                    timestr, Ec.Slave, Ec.Index, Ec.data.AbortCode, ec_soeerror2string(Ec.data.str.ErrorCode));            
            break;
         }
         case EC_ERR_TYPE_MBX_ERROR:
         {
//            sprintf(estring, "%s MBX slave:%d error:%4.4x %s\n", 
//                    timestr, Ec.Slave, Ec.data.str.ErrorCode, ec_mbxerror2string(Ec.data.str.ErrorCode));             
            break;
         }
         default:
         {
//            sprintf(estring, "%s error:%8.8x\n", 
//                    timestr, Ec.data.AbortCode);
         }
      }
      return (char*) estring;
   }
