...
//#define PDO_SEND_QUEUE_ELEMENT_SIZE ETH_HEADERSIZE + EC_HEADERSIZE + EC_WKCSIZE + EC_HEADERSIZE + 4 + LEN
...
void ecx_set_pdo_queue(ecx_portt *port, uint32 len, uint32 count) 
{
	cycle_queue_create(&(port->pdo_send_queue),port->pdo_send_buf,ETH_HEADERSIZE + EC_HEADERSIZE + EC_WKCSIZE + EC_HEADERSIZE + sizeof(int64) + len,count,0);
}
...
int ecx_outframe(ecx_portt *port, int idx, int stacknumber)
{
   ...
   if((*stack->txbuf)[idx][0x10] == 0x0C){
      buffer = cycle_queue_push_empty(&(port->pdo_send_queue));
	  if(buffer != NULL && lp == port->pdo_send_queue.element_size){
	     memcpy((uint8*)&buffer[0], (*stack->txbuf)[idx], lp);
	  }
   	}else{
      buffer = (uint8 *)(DMATxDescToSet->Buffer1Addr);
	  memcpy((uint8*)&buffer[0], (*stack->txbuf)[idx], lp);
	  /* Prepare transmit descriptors to give to DMA*/ 
	  ETH_Prepare_Transmit_Descriptors(lp);
   	}
...
int ecx_pdo_outframe(ecx_portt *port)
{
	uint8 *buffer =  (uint8 *)(DMATxDescToSet->Buffer1Addr);
	if (cycle_queue_peek(&(port->pdo_send_queue), buffer, 0) == 0) {
		
        ETH_Prepare_Transmit_Descriptors(port->pdo_send_queue.element_size);
		cycle_queue_pop(&(port->pdo_send_queue), NULL);
    }
	return 0;
}
...
//      pthread_mutex_lock( &(port->tx_mutex) );
	  ...
	  memcpy((uint8*)&buffer[0], &(port->txbuf2), port->txbuflength2);
	/* Prepare transmit descriptors to give to DMA*/ 

	  ETH_Prepare_Transmit_Descriptors(port->txbuflength2);
...
static int ecx_recvpkt(ecx_portt *port, int stacknumber)
{
   ...
   if (ETH_CheckFrameReceived()){ 
    ...
    bytesrx = frame.length;

	if (bytesrx > lp)
	{
	  bytesrx = lp;
	}
	memcpy(*stack->tempbuf, (uint8 *)frame.buffer, bytesrx);
...
#ifdef EC_VER1
void ec_set_pdo_queue(uint32 len, uint32 count)
{
	ecx_set_pdo_queue(&ecx_port,len,count);
}
...
int ec_pdo_outframe(void)
{
   return ecx_pdo_outframe(&ecx_port);
}
