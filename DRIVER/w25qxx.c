...
{ 
	...
	{	
		...
		SPI3_ReadWriteByte((u8)((secpos*4096)>>16));  	//发送24bit地址    
		SPI3_ReadWriteByte((u8)((secpos*4096)>>8));   
		SPI3_ReadWriteByte((u8)(secpos*4096));   
		for(i=0;i<4096;i++)
		{ 
				W25QXX_BUF[i]=SPI3_ReadWriteByte(0XFF);   	//循环读数  
		}
		W25QXX_CS=1;
		...
		if(secpos*4096 == SAVE_PROTECT_START_ADDRESS)
		{
			W25QXX_Erase_Sector(SAVE_PROTECT_BACKUP_ADDRESS/4096);										//擦除保护区域的备份扇区
			W25QXX_Write_NoCheck(W25QXX_BUF, SAVE_PROTECT_BACKUP_ADDRESS, 4096);			//写入保护区域的备份扇区
		}
...
{ 
	...
	{//备份和恢复保护区域的扇区数据和备份扇区数据
		
		/*读取保护区域的校验字，进行校验及恢复*/
		W25QXX_CS = 0;                            									//使能器件   
		SPI3_ReadWriteByte(W25X_ReadData);         									//发送读取命令   
		SPI3_ReadWriteByte((u8)((SAVE_PROTECT_END_ADDRESS)>>16));  	//发送24bit地址    
		SPI3_ReadWriteByte((u8)((SAVE_PROTECT_END_ADDRESS)>>8));   
		SPI3_ReadWriteByte((u8)SAVE_PROTECT_END_ADDRESS);   
		protectValue = SPI3_ReadWriteByte(0XFF);											//读取保护字节
		W25QXX_CS = 1;
		...
		{//恢复保护区域数据
			W25QXX_CS = 0;                            												//使能器件   
			SPI3_ReadWriteByte(W25X_ReadData);         												//发送读取命令   
			SPI3_ReadWriteByte((u8)((SAVE_PROTECT_BACKUP_ADDRESS)>>16));  		//发送24bit地址    
			...
			{ 
					W25QXX_BUFFER[i] = SPI3_ReadWriteByte(0XFF);   								//循环读数  
			}
			W25QXX_CS = 1;
			
			if(W25QXX_BUFFER[4096 - 1] == SAVE_PROTECT_VALUE)
			{
				W25QXX_Erase_Sector(SAVE_PROTECT_START_ADDRESS/4096);										//擦除保护区域扇区
				W25QXX_Write_NoCheck(W25QXX_BUFFER, SAVE_PROTECT_START_ADDRESS, 4096);	//写入保护区域扇区
			}
		}
		else
		{//恢复保护区域的备份扇区数据
			W25QXX_CS = 0;                            									//使能器件   
			SPI3_ReadWriteByte(W25X_ReadData);         									//发送读取命令   
			SPI3_ReadWriteByte((u8)((SAVE_PROTECT_BACKUP_END_ADDRESS)>>16));  	//发送24bit地址    
			...
			if(protectValue != SAVE_PROTECT_VALUE)
			{
				W25QXX_CS = 0;                            												//使能器件   
				SPI3_ReadWriteByte(W25X_ReadData);         												//发送读取命令   
				SPI3_ReadWriteByte((u8)((SAVE_PROTECT_START_ADDRESS)>>16));  		//发送24bit地址    
				...
				if(W25QXX_BUFFER[4096 - 1] == SAVE_PROTECT_VALUE)
				{
					W25QXX_Erase_Sector(SAVE_PROTECT_BACKUP_ADDRESS/4096);										//擦除保护区域扇区
					W25QXX_Write_NoCheck(W25QXX_BUFFER, SAVE_PROTECT_BACKUP_ADDRESS, 4096);		//写入保护区域扇区
				}
			}
		}
	}
...
{	
	...
	{
		W25QXX_Erase_Sector(SAVE_PROTECT_BACKUP_ADDRESS/4096);										//擦除保护区域的备份扇区
		W25QXX_Write_NoCheck(W25QXX_BUFFER, SAVE_PROTECT_BACKUP_ADDRESS, 4096);			//写入保护区域的备份扇区
	}
}
